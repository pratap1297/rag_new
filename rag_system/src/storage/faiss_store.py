...
class FAISSStore:
    ...
    def _cleanup_deleted_vectors(self):
        """Clean up deleted vectors from the index and mappings"""
        try:
            ...
            # Get the FAISS indices for deleted vectors
            deleted_indices = {
                idx for idx, vector_id in self.index_to_id.items()
            ...
            # Create a new index without the deleted vectors
            new_index = faiss.IndexFlatIP(self.dimension)
            
            # Get all vectors from the current index
            all_vectors = self.index.reconstruct_n(0, self.index.ntotal)
            
            # Add only non-deleted vectors to the new index
            new_index_to_id = {}
            new_id_to_metadata = {}
            current_idx = 0
            ...
            for old_idx in range(self.index.ntotal):
                if old_idx not in deleted_indices:
                    ...
                    new_index.add(all_vectors[old_idx:old_idx+1])
                    new_index_to_id[current_idx] = vector_id
                    new_id_to_metadata[vector_id] = self.id_to_metadata[vector_id]
                    current_idx += 1
            
            # Update the index and mappings
            ...
            logging.info(f"Cleaned up {len(deleted_indices)} deleted vectors")
    ...
    def delete_vectors(self, vector_ids: List[int]):
        """Delete vectors from the index and clean up mappings"""
        try:
